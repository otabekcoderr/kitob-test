...
let booksStr = booksMatch[1];
booksStr = booksStr.replace(/questionCount:\s*\d+/g, "questionCount: 5");
...
const outQuestions = [];
const ids = Array.from(booksStr.matchAll(/id:\s*'([^']+)'/g)).map(m => m[1]);

for (const bookId of ids) {
    if (deepQs[bookId]) {
        deepQs[bookId].forEach((q, idx) => {
            outQuestions.push({
                id: `q-${bookId}-${idx+1}`,
                bookId: bookId,
                question: q.q,
...
let qsJs = "export const questions = [\n";
outQuestions.forEach(q => {
    qsJs += `  {
    id: '${q.id}',
    bookId: '${q.bookId}',
...
fs.writeFileSync('js/data.js', `export const books = ${booksStr};\n\n${qsJs}`, 'utf8');
console.log("Successfully updated data.js with complex questions!");
